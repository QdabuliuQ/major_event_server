...
exports.getCollectVideo = (req, res) => {
    ...
    const sqlStr = `select ev_vcr.*, ev_v.cover_img, ev_v.title, ev_v.time as pub_date, ev_v.video_url, ev_u.nickname,  ev_u.user_pic from ev_video_collect_record ev_vcr join ev_videos ev_v on ev_vcr.video_id = ev_v.id and ev_v.is_delete='0' inner join ev_users ev_u on ev_v.user_id=ev_u.id where ev_vcr.user_id=? order by ev_vcr.time desc limit ?,?`
    db.query(sqlStr, [
        req.user.id,
