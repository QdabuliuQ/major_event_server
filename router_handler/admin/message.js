	...
	let {stateSql,typeSql,timeSql,valSql} = createConditionSql([
	    {
	        ...
	        name: 'val',
	        type: 'like',
	        fields: ['ev_ml.msg_id','ev_ml.room_id','ev_ml.to_id','ev_ml.from_id','ev_ml.resource', 'ev_u_re.nickname', 'ev_u_se.nickname', 'ev_mr.user_id', 'ev_mr.send_id']
	    }
	], req.query)
	const sqlStr = `select ev_mr.*, ev_u_re.nickname re_nickname, ev_u_re.user_pic re_user_pic, ev_u_se.nickname se_nickname, ev_u_se.user_pic se_user_pic, ev_ml.type, ev_ml.resource from ev_message_report ev_mr inner join ev_users ev_u_re on ev_mr.user_id=ev_u_re.id left join ev_users ev_u_se on ev_mr.send_id=ev_u_se.id left join ev_message_list ev_ml on ev_mr.msg_id=ev_ml.msg_id where ${stateSql} and ${typeSql} and ${timeSql} and ${valSql} order by ev_mr.time desc`
	db.query(sqlStr, (err, results) => {
		if(err) return res.cc(err)
		for(let item of results) {
		...
		let data = results
		const sqlStr = `select count(*) as count from ev_message_report ev_mr inner join ev_users ev_u_re on ev_mr.user_id=ev_u_re.id left join ev_users ev_u_se on ev_mr.send_id=ev_u_se.id left join ev_message_list ev_ml on ev_mr.msg_id=ev_ml.msg_id where ${stateSql} and ${typeSql} and ${timeSql} and ${valSql}`
		db.query(sqlStr, (err, results) => {
			if(err) return res.cc(err)
			let count = results[0].count
			res.send({
...
exports.updateReportState = (req, res) => {
	const sqlStr = 'select * from ev_message_report where rep_id=? and state="1"'
	db.query(sqlStr, req.body.rep_id, (err, results) => {
		if(err) return res.cc(err)
		if(results.length != 1) return res.cc('更新状态失败')
		const sqlStr = 'update ev_message_report set state=? where rep_id=?'
		db.query(sqlStr, [
			req.body.state,
			req.body.rep_id
		], (err, results) => {
			...
			if(req.body.state == '2') {
				const sqlStrs = 'update ev_message_report set state="2" where msg_id=? and state="1"; update ev_message_list set state="2" where msg_id=?'
				db.query(sqlStrs, [
					req.body.msg_id,
					req.body.msg_id
				], (err, results) => {
